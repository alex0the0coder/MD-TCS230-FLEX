...
MD_TCS230_FLEX	CS(S2_OUT, S3_OUT, OE_OUT);
	...
	case 3:	// wait for a read to complete
		if (CS.available()) 
		{
			...
			case READ_VAL:	
				CS.getRGB(&rgb);
				Serial.print(F("\nRGB is ["));
				Serial.print(rgb.value[TCS230_FLEX_RGB_R]);
				Serial.print(F(","));
				Serial.print(rgb.value[TCS230_FLEX_RGB_G]);
				Serial.print(F(","));
				Serial.print(rgb.value[TCS230_FLEX_RGB_B]);
				Serial.print(F("]"));
				break;
			}
			state++;
