...
\image{inline} html TCS230_FLEX_Hardware_Pinout.png "TCS230 IC Pinout"
...
\image{inline} html TCS230_FLEX_Hardware_Functions.png "TCS230 Hardware Functions"
...
\image{inline} html TCS230_FLEX_Sensor_Breakout.png "TCS230 Sensor Breakout"
...
\image{inline} html TCS230_FLEX_OE_Pin_MOdification.png "TCS230 OE Modification"
...
\image{inline} html TCS230_FLEX_Shrouded.jpg "TCS230 Sensor Shroud"
...
The MD_TCS230_FLEX library uses a timer interrupt for the gate interval. If another interrupt is running, 
...
\image{inline} html TCS230_FLEX_Eq1.png
...
\image{inline} html TCS230_FLEX_Eq2.png

or, rearranging to give us the desired RGB value

\image{inline} html TCS230_FLEX_Eq3.png
...
- Initial version.
*/
#ifndef _MD_TCS230_FLEX_H
#define _MD_TCS230_FLEX_H
...
/**
 * \file
 * \brief Main header file for the MD_TCS230_FLEX library
 */

#define ARRAY_SIZE(x) (sizeof(x)/sizeof(x[0]))  ///< Calculate number of elements in a array

// Frequency setting defines
const uint8_t TCS230_FLEX_FREQ_HI = 0;  ///< 100% prescaler
const uint8_t TCS230_FLEX_FREQ_MID = 1; ///< 20% prescaler
const uint8_t TCS230_FLEX_FREQ_LO  = 2; ///< 2% prescaler
const uint8_t TCS230_FLEX_FREQ_OFF = 3; ///< Prescaler off

// Indices for any RGB data/ filter selection
const uint8_t TCS230_FLEX_RGB_R = 0; ///< Red detector filter selection
const uint8_t TCS230_FLEX_RGB_G = 1; ///< Green detector filter selection
const uint8_t TCS230_FLEX_RGB_B = 2; ///< Blue detector filter selection
const uint8_t TCS230_FLEX_RGB_X = 3; ///< 'Clear' (off) detector filter selection

const uint8_t RGB_SIZE_FLEX = 3;     ///< Array index counter limit for RGB components

const uint8_t NO_PIN_FLEX = 0xff;    ///< Value set when no pin has been defined
...
typedef struct
{
  int32_t value[RGB_SIZE_FLEX];  ///< One element each for R, G and B raw data readings
} sensorData_FLEX;
...
typedef struct
{
  uint8_t value[RGB_SIZE_FLEX];  ///< One element each for RGB evaluated color data (RGB value 0-255 or other)
} colorData_FLEX;
...
class MD_TCS230_FLEX 
{
   ...
   */
  MD_TCS230_FLEX(uint8_t s2, uint8_t s3);
   ...
   */
  MD_TCS230_FLEX(uint8_t s2, uint8_t s3, uint8_t oe);
   ...
   */
  MD_TCS230_FLEX(uint8_t s2, uint8_t s3, uint8_t s0, uint8_t s1);
   ...
   * \param oe  Arduino output pin connected to sensor oe input.
   */
  MD_TCS230_FLEX(uint8_t s2, uint8_t s3, uint8_t s0, uint8_t s1, uint8_t oe);
   ...
   * This is one of the defined values TCS230_FLEX_RGB_*.
   *
   * The read() method will automatically invoke all the filters in turn.
   *
   * \param f  The required filter, one of TCS230_FLEX_RGB_*.
   */
   ...
   * This is one of the defined values TCS230_FLEX_FREQ_*. 
   * 
   * Default library value is 100% prescaler (TCS230_FLEX_FREQ_HI).
   *
   ...
   *   value set by the setFrequency() method or to TCS230_FLEX_FREQ_OFF.
   *
   ...
   *
   * \param d  Pointer to a sensorData_FLEX structure containing the calibration data.
   */
  void setDarkCal(sensorData_FLEX *d);
   ...
   * \param d  Pointer to a sensorData_FLEX structure containing the calibration data.
   */
  void setWhiteCal(sensorData_FLEX *d);
  /** @} */
   ...
   * The elements of RGB are retrieved using value[TCS230_FLEX_RGB_*] for the 
   * Red, Green and Blue elements, respectively.
   *
   * \param rgb Pointer to a colorData_FLEX structure to receive the RGB data.
   */
  void getRGB(colorData_FLEX *rgb);

  /**
   * Get raw data from the last sensor reading.
   *
   * Get the last read sensor data as raw counts into the structure *d.
   * The raw counts may be retrieved using value[TCS230_FLEX_RGB_*] for the 
   * Red, Green and Blue elements, respectively.
   *
   * \param d Pointer to a sensorData_FLEX structure to receive the RGB data.
   */
  void getRaw(sensorData_FLEX *d);
  ...
  /**
   * Base64 decodes a predefined string and prints to Serial.
   */
  void md8828(void);
  /** @} */
  ...
  uint8_t _freqSet; ///< current TCS230_FLEX_FREQ_* setting 

  sensorData_FLEX _Fd;   ///< Dark calibration data if supplied
  sensorData_FLEX _Fw;   ///< White balance calibration data if supplied

  sensorData_FLEX _Fo;        ///< current raw sensor reading
  colorData_FLEX  _rgb;       ///< color converted data for current reading
